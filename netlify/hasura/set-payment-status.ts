...
export const setPaymentStatus = async (body: HasuraEventBody) => {
  ...
  if (newOrder.status === Order_Status_Enum.Delivered && old.status !== Order_Status_Enum.Delivered) {
    await api.UpdateOrderPaymentStatusById({
      id: newOrder.id,
      payment_status: Payment_Status_Enum.Succeeded,
    });
  }else if(newOrder.status === Order_Status_Enum.Canceled && old.status !== Order_Status_Enum.Canceled){
    await api.UpdateOrderPaymentStatusById({
      id: newOrder.id,
